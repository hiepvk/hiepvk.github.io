...
$(document).ready(function() {
    ...
    $.ajax({
        type: "GET",
        url: 'https://hiepvk.github.io/' + key + ".m3u",
        success: function(message, text, response) {
            ...
            for (let i = 0; i < links.length; i++) {
                ...
                if ($(window).width() > 640) {
                    if (window.localStorage.getItem(links[i]) == lst[i][0]) {
                        $("#menu").append(`<li><p><span title=${links[i]}>${lst[i][0]}</span></p></li>`);
                    } else {
                        $("#menu").append(`<li><p><span title=${links[i]}>${lst[i][0]}</span></p></li>`);
                    }
                } else {
            ...
            for (let i of Object.keys(localStorage).filter(x => !localkey.includes(x))) {
                if ($(window).width() > 640) {
                    $("#channelcontent").append(`<li><p><span title=${i}>${localStorage[i]}</span></p></li>`);
                } else {
                    $("#channelcontent").append(`<li><p><span title=${i}>${localStorage[i]}</span></p></li>`);
                }
            }
    ...
    $("#github").on({
        ...
        click: function() {
            window.open("http://hiepvk.github.io/tv");
        },
        mouseleave: function() {
            $(this).css({ "opacity": 0.5 })
        }
